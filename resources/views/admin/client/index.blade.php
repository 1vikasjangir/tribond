...
@section('content')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-5 pb-3 mb-3 heading_header">
        ...
        <div class="right">
            {{-- @if (auth()->user()->hasAnyPermission(['add_client'])) --}}
            <a href="{{ route('client.add') }}" class="btn btn-primary create_new">Add New</a>
            {{-- @endif --}}
            ...
            <div id="collapseOne" class="accordion-collapse collapse filter_tab" aria-labelledby="headingOne"
                ...
                <div class="filter_card">
                    <div class="accordion-body">
                        ...
                        <form autocomplete="off" id="blog-filters" action="{{ route('client.index') }}" method="POST">
                            @csrf
                            @method('GET')
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <a href="{{ route('client.index') }}" class="btn btn-dark">Reset</a>
                                </div>
                            </div>
                        </form>
                    </div>
                ...
                <tbody>
                    @forelse ($clients as $client)
                        <tr>
                            ...
                            <td><img src="{{ $client->logo }}" width="100px" height="100px"
                                    alt="image"></td>
                            <td>{{ $client->sort_order }}</td>
                            <td> {{ $client->created_at->format('d-m-Y') }}</td>
                            <td>
                                <a href="javascript:;" data-id="{{ $client->id }}" class="change-status-btn">
                                    <i id="active_{{ $client->id }}" style="display:{{ $client->status == '1' ? 'block' : 'none'}};" class="far fa-circle-check md-24 text-success" data-toggle="tooltip" title="Active"></i>
                                    <i id="deactive_{{ $client->id }}" style="display:{{ $client->status == '0' ? 'block' : 'none'}};" class="far fa-times-circle md-24 text-danger" data-toggle="tooltip" title="Deactive"></i>
                                </a>
                            </td>

                            <td>
                                <a href="javascript:;" data-id="{{ $client->id }}" id="delete_{{ $client->id }}" class="change-delete-btn">
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if(data.status == "1"){
                            ...
                            $('#active_'+client_id).show()
                            $('#deactive_'+client_id).hide()
                            $("#alert").css('opacity', '1').html(data.success).fadeIn(2000).fadeOut(4000);
                        } else {
                            // Code in the case checkbox is NOT checked.
                            $('#deactive_'+client_id).show()
                            $('#active_'+client_id).hide()
                            $("#alert").css('opacity', '1').html(data.success).fadeIn(2000).fadeOut(4000);
                        }
                    },
                });
